...
int main(int argc, char *argv[])
{
  ...
  */
  ...
  while(1)
  {
    ...
    sentBytes = recvfrom(serverSocket, &tempProtocol, sizeof(tempProtocol), 0, ptr->ai_addr, &ptr->ai_addrlen);
    if (sentBytes == -1)
    {
    ...
    if (client.ipAddress != inet_ntoa(ipAddrs) || client.clientID != ntohl(cProtocol->id))
    {
    ...
    if (sentBytes == sizeof(cMessage))
    {
      printf("Calcmessage recieved \n");
      cMessage = (calcMessage *) tempProtocol;
      ...
      printf("Type: %d Message: %d Protocol: %d Majorversion %d\n", cMessage->type, cMessage->message,
        cMessage->protocol, cMessage->major_version);
    ...
    else if (sentBytes == sizeof(cProtocol))
    {
